...
use crate::ai::{AIGenerator, GeminiChat, OllamaChat, OpenAIChat};
        ...
        match expr {
            ...
            Expression::GenerateContent(provider_expr, prompt_expr) => {
                ...
                let provider_enum = match provider_name_str.as_str() {
                    ...
                    "gemini" => crate::ai::AIProvider::Gemini,
                    _ => {
...
/// AuraScriptのデフォルトスクリプトを実行します。
/// AIとの連携やファイル操作の例が含まれています。
pub async fn run_default_script(config: Config) {
    let script = r#"
