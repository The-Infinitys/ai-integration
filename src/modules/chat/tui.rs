...
impl TuiApp {
    pub fn new(ollama_base_url: String, default_ollama_model: String) -> Self {
        ...
        Self {
            ...
            input: String::new(),
            messages: vec![
                ChatMessage { role: ChatRole::System, content: format!("Default Ollama Model: {}", default_ollama_model) },
                ChatMessage { role: ChatRole::System, content: "AI Integration Chat Session".to_string() },
                ChatMessage { role: ChatRole::System, content: "Type '/exit' to quit.".to_string() },
                ChatMessage { role: ChatRole::System, content: "Type '/model <model_name>' to change model.".to_string() },
                ChatMessage { role: ChatRole::System, content: "Type '/list models' to list available models.".to_string() },
                ChatMessage { role: ChatRole::System, content: "Type '/revert' to undo the last turn.".to_string() },
            ],
            input_scroll: 0,
            should_quit: false,
    ...
    async fn run_app_loop(&mut self, terminal: &mut Terminal<CrosstermBackend<Stdout>>) -> Result<()> {
        ...
        loop {
            ...
            if let Some(tui_event) = self.event_receiver.recv().await {
                match tui_event {
                    TuiEvent::Input(key_event) => self.handle_input_event(key_event, terminal.size()?.width).await?,
    ...
    fn ui(&mut self, frame: &mut ratatui::Frame) {
        ...
        for message in &self.messages {
            ...
            list_items.extend(self.create_list_items(&message.content, prefix, color, size.width));
        }

        if !self.ai_response_buffer.is_empty() {
            list_items.extend(self.create_list_items(&self.ai_response_buffer, "AI: ", Color::Green, size.width));
        }
        if !self.tool_output_buffer.is_empty() {
            list_items.extend(self.create_list_items(&self.tool_output_buffer, "Tool: ", Color::Magenta, size.width));
        }

        if !list_items.is_empty() {
            self.message_list_state.select(Some(list_items.len() - 1));
        }

        let list = List::new(list_items).block(messages_block);
        frame.render_stateful_widget(list, main_layout[0], &mut self.message_list_state);

        let input_title = if self.is_ai_replying { "Input (AI is replying...)" } else { "Input" };
        let input_block = Block::default().borders(Borders::ALL).title(input_title);
        let input_text = Paragraph::new(self.input.as_str())
            .style(Style::default().fg(if self.is_ai_replying { Color::DarkGray } else { Color::White }))
            .block(input_block)
        ...
        if !self.is_ai_replying {
            let cursor_x = main_layout[1].x + self.input.width() as u16 - self.input_scroll + 1;
            frame.set_cursor_position(Position::new(cursor_x, main_layout[1].y + 1));
        }

        let status_text = Paragraph::new(self.status_message.as_str()).style(Style::default().fg(self.status_text_color));
        frame.render_widget(status_text, main_layout[2]);
    }
    
    fn create_list_items<'a>(&self, content: &'a str, prefix: &'a str, color: Color, width: u16) -> Vec<ListItem<'a>> {
        let mut items = Vec::new();
        let wrapped_content = textwrap::wrap(content, (width as usize).saturating_sub(4));
    ...
    async fn handle_input_event(&mut self, key_event: KeyEvent, terminal_width: u16) -> Result<()> {
        ...
        let input_width = self.input.width() as u16;
        ...
        } else if self.input_scroll > 0 && input_width < self.input_scroll + input_area_width {
            self.input_scroll = input_width.saturating_sub(input_area_width);
        }
    ...
    async fn handle_enter(&mut self) {
        ...
        if input_copy.starts_with('/') {
            ...
            self.messages.push(ChatMessage { role: ChatRole::User, content: input_copy.clone() });
...
",
    ...
    fn handle_error(&mut self, e: String) {
        ...
        self.messages.push(ChatMessage { role: ChatRole::System, content: format!("Error: {}", e) });
...
");
        ...
        self.messages.push(ChatMessage { role: ChatRole::System, content: model_list_message });
    ...
    async fn flush_ai_buffer_to_messages(&mut self) {
        if !self.ai_response_buffer.is_empty() {
            ...
            self.chat_session.add_assistant_message_to_history(content.clone()).await;
            self.messages.push(ChatMessage { role: ChatRole::Assistant, content });
    ...
    fn flush_tool_buffer_to_messages(&mut self) {
        if !self.tool_output_buffer.is_empty() {
            self.messages.push(ChatMessage { role: ChatRole::System, content: self.tool_output_buffer.clone() });
